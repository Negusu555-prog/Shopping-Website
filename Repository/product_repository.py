...
async def delete_product(product_id: int) -> bool:
    # שאילתת מחיקה
    q = "DELETE FROM Products WHERE ProductID=:product_id"
    # הפקודה execute מחזירה את מספר השורות שהושפעו (נמחקו)
    result = await database.execute(q, values={"product_id": product_id})
    return result > 0